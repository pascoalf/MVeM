...
ASV_rarefaction <- read.delim("ASV_table.tsv", header = TRUE, row.names = 1, sep = "\t", check.names = FALSE)
...
rownames(ASV_rarefaction) <- rownames(read.delim("ASV_table.tsv", header = TRUE, sep = "\t", check.names = FALSE, row.names = 1))
