...
![Quality profiles of forward reads - 5 files](results/quality_forward.png)

![Quality profiles of reverse reads - 5 files](results/quality_reverse.png)
...
![Aggregate quality plot example for forward reads](results/aggregate_forward.png)


![Aggregate quality plot example for reverse reads](results/aggregate_reverse.png)
...
- Likewise, we can break ties up to family level (that is, assign LCA up to order level).
-   Note: if genera from different orders are tied, we assign to **Uncertain**.
...
*Note*: ASVs removed during contamination removal process are marked as NAs in the wide format table.
...
![Rarefaction curve example](results/rarefaction_curve_example.png)
...
![Alpha diversity - boxplot example](results/alpha_boxplot_example.png)
